...
// Do if is admin only.
if( is_admin() ) {
	add_action( 'load-post.php', 'di_multipurpose_post_meta_box_first_func' );
	add_action( 'load-post-new.php', 'di_multipurpose_post_meta_box_first_func' );
}

if( ! function_exists( 'di_multipurpose_post_meta_box_first_func' ) ) {
	/**
	 * Calls the class on the post edit screen.
	 */
	function di_multipurpose_post_meta_box_first_func() {
		new Di_Multipurpose_Post_Meta_Box_Main_Class();
	}
}

if( ! class_exists( 'Di_Multipurpose_Post_Meta_Box_Main_Class' ) ) {
	/**
	 * The post metabox class.
	 */
	class Di_Multipurpose_Post_Meta_Box_Main_Class {
		...
		public function add_meta_box( $post_type ) {
			if ( $post_type == 'post' ) {
				add_meta_box(
					'di_multipurpose_post_meta_box_cntnr',
					__( 'Di Multipurpose Theme Options for this Post', 'di-multipurpose' ),
		...
		public function render_meta_box_content( $post ) {
			...
			wp_nonce_field( 'di_multipurpose_post_meta_bx_key', 'di_multipurpose_post_meta_bx_key_nonce' );
			...
			<?php
			do_action( 'di_multipurpose_post_metabox_render', $post );
		}
		...
		public function save( $post_id ) {
			...
			if( ! isset( $_POST['di_multipurpose_post_meta_bx_key_nonce'] ) ) {
				return $post_id;
			}
	 
			$nonce = sanitize_text_field( wp_unslash( $_POST['di_multipurpose_post_meta_bx_key_nonce'] ) );
	 
			// Verify that the nonce is valid.
			if( ! wp_verify_nonce( $nonce, 'di_multipurpose_post_meta_bx_key' ) ) {
				return $post_id;
			}
			...
			do_action( 'di_multipurpose_post_metabox_save', $post_id );
