...
if( ! function_exists( 'di_multipurpose_post_navigation' ) ) {
	/**
	 * Post navigation.
	 * @return [type] [description]
	 */
	function di_multipurpose_post_navigation() {
...
if( ! function_exists( 'di_multipurpose_posts_pagination' ) ) {
	/**
	 * Posts navigation or pagination.
	 * @return [type] [description]
	 */
	function di_multipurpose_posts_pagination() {
		if( get_theme_mod( 'display_archive_pagination', 'nextprev' ) == 'nextprev' ) {
			...
			?>
				...
				<nav class="navigation post-navigation dipostsnav" role="navigation">
					<div class="nav-previous"><?php next_posts_link( __( '&larr; Older Entries ', 'di-multipurpose' ) ); ?></div>
					<div class="nav-next"><?php previous_posts_link( __( 'Newer Entries &rarr;', 'di-multipurpose' ) ); ?></div>
				</nav>
				<div class="clearfix"></div>
			<?php
			}
			...
			the_posts_pagination( array(
				'prev_text' => __( '&laquo;', 'di-multipurpose' ),
				'next_text' => __( '&raquo;', 'di-multipurpose' ),
			) );
		}
	}
}

if( ! function_exists( 'di_multipurpose_breadcrumbs' ) ) {
	/**
	 * Breadcrumbs.
	 * @return [type] [description]
	 */
	function di_multipurpose_breadcrumbs() {
		$custom_taxonomy = '';
		...
		if( ! is_front_page() ) {
			...
			echo '<div class="col-md-12"><nav aria-label="breadcrumb" class="breadcrumb-otr"><ol class="breadcrumb small">';

			// Home page.
			echo '<li class="breadcrumb-item"><a href="' . esc_url( home_url( '/' ) ) . '">' . esc_html__( 'Home', 'di-multipurpose' ) . '</a></li>';

			if ( is_home() ) {

				echo '<li class="breadcrumb-item active">' . esc_html__( 'Blog', 'di-multipurpose' ) . '</li>';
				...
				// Category page.
				echo '<li class="breadcrumb-item active">' . esc_html__( 'Category: ', 'di-multipurpose' ) . single_cat_title( '', false ) . '</li>';
				...
				foreach ( $anc as $ancestor ) {
					$parents .= '<li class="breadcrumb-item"><a href="' . esc_url( get_permalink( $ancestor ) ) . '">' . esc_html( get_the_title( $ancestor ) ) . '</a></li>';
				}

				// Display parent pages.
				echo $parents; // Escaped.
				...
				echo '<li class="breadcrumb-item active">' . esc_html__( 'Tag: ', 'di-multipurpose' ) . esc_html( $get_term_name ) . '</li>';
				...
				echo '<li class="breadcrumb-item active">' . esc_html__( 'Author: ', 'di-multipurpose' ) . get_the_author() . '</li>';
			} elseif( get_query_var( 'paged' ) ) {
				echo '<li class="breadcrumb-item active">' . esc_html__( 'Page: ', 'di-multipurpose' ) . esc_html( get_query_var( 'paged' ) ) . '</li>';
			} elseif( is_search() ) {
				echo '<li class="breadcrumb-item active">' . esc_html__( 'Search: ', 'di-multipurpose' ) . get_search_query() . '</li>';
			} elseif( is_404() ) {
				echo '<li class="breadcrumb-item active">' . esc_html__( 'Error 404', 'di-multipurpose' ) . '</li>';
			} else {
				echo '<li class="breadcrumb-item active">' . esc_html__( 'Untitled', 'di-multipurpose' ) . '</li>';
			}
...
if( ! function_exists( 'di_multipurpose_post_thumbnail' ) ) {
	/**
	 * Post thumbnail section.
	 * @return [type] [description]
	 */
	function di_multipurpose_post_thumbnail() {
...
if( ! function_exists( 'di_multipurpose_nav_fallback' ) ) {
	/**
	 * Main menu fallback.
	 * @param  [type] $args [description]
	 * @return [type]       [description]
	 */
	function di_multipurpose_nav_fallback( $args ) {
		extract( $args );
		$output = null;
		if( $container ) {
			$output = '<' . $container;
		...
		$output .= '<li class="menu-item menu-item-type-post_type menu-item-object-page current-menu-item page_item current_page_item active"><a class="nav-link" href="' . esc_url( home_url( '/' ) ) . '">'. __( 'Home', 'di-multipurpose' ) .'</a></li>';
		
		if( current_user_can( 'manage_options' ) ) {
			$output .= '<li class="menu-item menu-item-type-custom"><a class="nav-link" target="_blank" href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '">'. __( 'Edit Menu', 'di-multipurpose' ) .'</a></li>';
		}
		...
		echo $output; // DB values already escaped.
	}
}


if( ! function_exists( 'di_multipurpose_comments' ) ) {
	/**
	 ...
	 * @return [type]          [description]
	 */
	function di_multipurpose_comments( $comment, $args, $depth ) {
	?>
				...
				<div id="comment-<?php comment_ID(); ?>" class="dimedia" >
					...
					<div class="dimedia-body">
						...
						<?php if( $comment->comment_approved == '0' ) { ?>
							<p class="alert alert-info" ><?php esc_html_e( 'Your comment is awaiting approval.', 'di-multipurpose' ); ?></p>
						<?php } ?>
						...
						<small>
							<a itemprop="url" href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>"> 
								...
								printf(
									...
									esc_html__( '%1$s at %2$s', 'di-multipurpose' ),
						...
						<?php comment_reply_link( array_merge( $args,
							array(
								...
								'reply_text' => __( 'Reply', 'di-multipurpose' ),
								)
						) );
						?>
						
						<?php edit_comment_link( __( 'Edit', 'di-multipurpose' ), '', '' ) ?>
						</small>
